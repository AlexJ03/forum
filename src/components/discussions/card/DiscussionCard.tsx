...
const DiscussionCard: FC<Omit<IDiscussion, "answers" | "category">> = ( { name, date, fromUser } ) => {
    ...
    const path = name && encrypt( name );

    return (
        <Box sx={{ marginBottom: "30px", cursor: "pointer", background: "#e3e3e3", borderRadius: 5, width: "100%", minHeight: "200px", px: 5, py: 3, transition: "300ms", ":hover": { background: "#d3d3d3", transform: "translate(0, -10px)" } }} onClick={() => navigate( `/discussions/${path}` )} key={name}>
